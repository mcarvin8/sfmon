...
Currently wired into `monitor_salesforce_limits` (governor limit breaches) and `get_salesforce_incidents` (active Salesforce Trust API incidents on your org's pod); the underlying `sync_alerts()` API in [`slack_notify.py`](../src/sfmon/slack_notify.py) is generic and intended to be reused by other collectors over time.

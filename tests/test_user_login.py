...
class TestResetLoginGauges:
    def test_sets_all_gauges_to_zero(self):
        from sfmon.audit.user_login import reset_login_gauges
        mock_success = MagicMock()
        mock_failure = MagicMock()
        mock_unique = MagicMock()
        with patch("sfmon.audit.user_login.login_success_gauge", mock_success), \
             patch("sfmon.audit.user_login.login_failure_gauge", mock_failure), \
             patch("sfmon.audit.user_login.unique_login_attempts_gauge", mock_unique):
            reset_login_gauges()
...
class TestFetchLatestLoginLog:
    ...
    def test_returns_none_when_no_records(self):
        from sfmon.audit.user_login import fetch_latest_login_log
        with patch("sfmon.audit.user_login.query_records_all", return_value=[]):
    ...
    def test_returns_log_text_on_success(self):
        ...
        with patch("sfmon.audit.user_login.query_records_all", return_value=records), \
             patch("sfmon.audit.user_login.requests.get", return_value=mock_resp):
...
class TestProcessLoginLog:
    def test_counts_successes_and_failures(self):
        from sfmon.audit.user_login import process_login_log
...
class TestMonitorLoginEvents:
    def test_calls_process_when_log_data_present(self, mock_sf):
        from sfmon.audit.user_login import monitor_login_events
        log_text = "LOGIN_STATUS,USER_ID\nLOGIN_NO_ERROR,u01\n"
        with patch("sfmon.audit.user_login.reset_login_gauges") as m_reset, \
             patch("sfmon.audit.user_login.fetch_latest_login_log", return_value=log_text) as m_fetch, \
             patch("sfmon.audit.user_login.process_login_log") as m_process:
    ...
    def test_skips_process_when_no_log(self, mock_sf):
        from sfmon.audit.user_login import monitor_login_events
        with patch("sfmon.audit.user_login.reset_login_gauges"), \
             patch("sfmon.audit.user_login.fetch_latest_login_log", return_value=None), \
             patch("sfmon.audit.user_login.process_login_log") as m_process:
            monitor_login_events(mock_sf)
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.audit.user_login import monitor_login_events
        with patch("sfmon.audit.user_login.reset_login_gauges"), \
             patch("sfmon.audit.user_login.fetch_latest_login_log", side_effect=RuntimeError("fail")):
...
class TestGeolocation:
    ...
    def test_sets_gauge_for_location_with_geo(self, mock_sf):
        from sfmon.audit.user_login import geolocation
        locations = [self._make_location()]
        mock_gauge = MagicMock()
        with patch("sfmon.audit.user_login.query_records_all", side_effect=[locations, [{"Id": "u01", "Name": "Alice"}]]), \
             patch("sfmon.audit.user_login.geolocation_gauge", mock_gauge):
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.audit.user_login import geolocation
        with patch("sfmon.audit.user_login.query_records_all", side_effect=RuntimeError("fail")):
            geolocation(mock_sf, chunk_size=100)  # Should not raise

    def test_uses_default_chunk_size(self, mock_sf):
        from sfmon.audit.user_login import geolocation
        with patch("sfmon.audit.user_login.query_records_all", return_value=[]), \
             patch("sfmon.audit.user_login.geolocation_gauge"):
    ...
    def test_handles_key_error_in_record(self, mock_sf):
        ...
        with patch("sfmon.audit.user_login.query_records_all", return_value=[{"NoUserId": "xxx"}]):
            geolocation(mock_sf, chunk_size=100)  # Should not raise
    ...
    def test_chunks_user_lookups(self, mock_sf):
        ...
        with patch("sfmon.audit.user_login.query_records_all", side_effect=[locations, user_results[:2], user_results[2:]]), \
             patch("sfmon.audit.user_login.geolocation_gauge"):

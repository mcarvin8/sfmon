...
class TestHourlyReportExportRecords:
    ...
    def test_sets_gauge_for_valid_report_id(self, mock_sf):
        from sfmon.audit.report_export import hourly_report_export_records
        rows = [self._make_row()]
        report = [{"Id": "00O000000000001AAA", "Name": "My Report", "ReportTypeApiName": "AccountList"}]
        mock_gauge = MagicMock()
        with patch("sfmon.audit.report_export.parse_logs", return_value=iter(rows)), \
             patch("sfmon.audit.report_export.get_user_name", return_value="Alice"), \
             patch("sfmon.audit.report_export.query_records_all", return_value=report), \
             patch("sfmon.audit.report_export.hourly_report_export_metric", mock_gauge):
    ...
    def test_handles_uri_without_leading_slash(self, mock_sf):
        ...
        mock_gauge = MagicMock()
        with patch("sfmon.audit.report_export.parse_logs", return_value=iter(rows)), \
             patch("sfmon.audit.report_export.get_user_name", return_value="Bob"), \
             patch("sfmon.audit.report_export.query_records_all", return_value=report), \
             patch("sfmon.audit.report_export.hourly_report_export_metric", mock_gauge):
            hourly_report_export_records(mock_sf)
    ...
    def test_handles_none_logs(self, mock_sf):
        from sfmon.audit.report_export import hourly_report_export_records
        mock_gauge = MagicMock()
        with patch("sfmon.audit.report_export.parse_logs", return_value=None), \
             patch("sfmon.audit.report_export.hourly_report_export_metric", mock_gauge):
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.audit.report_export import hourly_report_export_records
        with patch("sfmon.audit.report_export.parse_logs", side_effect=RuntimeError("fail")), \
             patch("sfmon.audit.report_export.hourly_report_export_metric"):
    ...
    def test_falls_back_to_unknown_report_type_when_no_result(self, mock_sf):
        ...
        with patch("sfmon.audit.report_export.parse_logs", return_value=iter(rows)), \
             patch("sfmon.audit.report_export.get_user_name", return_value="Alice"), \
             patch("sfmon.audit.report_export.query_records_all", return_value=[]), \
             patch("sfmon.audit.report_export.hourly_report_export_metric", mock_gauge):
            hourly_report_export_records(mock_sf)

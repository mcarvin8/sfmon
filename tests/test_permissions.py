...
class TestUnassignedPermissionSets:
    def test_sets_gauge_per_record(self, mock_sf):
        from sfmon.tech_debt.permissions import unassigned_permission_sets
        records = [
            {"Id": "ps1", "Name": "EmptyPS1"},
            {"Id": "ps2", "Name": "EmptyPS2"},
        ]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.permissions.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.permissions.unused_permissionsets", mock_gauge):
    ...
    def test_empty_results(self, mock_sf):
        from sfmon.tech_debt.permissions import unassigned_permission_sets
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.permissions.query_records_all", return_value=[]), \
             patch("sfmon.tech_debt.permissions.unused_permissionsets", mock_gauge):
            unassigned_permission_sets(mock_sf)
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.tech_debt.permissions import unassigned_permission_sets
        with patch("sfmon.tech_debt.permissions.query_records_all", side_effect=RuntimeError("fail")):
            unassigned_permission_sets(mock_sf)  # Should not raise


class TestPermSetsLimitedUsers:
    def test_sets_gauge_per_record(self, mock_sf):
        from sfmon.tech_debt.permissions import perm_sets_limited_users
        records = [{"Id": "ps1", "Name": "LimitedPS", "expr0": "3"}]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.permissions.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.permissions.limited_permissionsets", mock_gauge):
...
class TestProfileAssignmentUnder5:
    def test_sets_gauge_per_record(self, mock_sf):
        from sfmon.tech_debt.permissions import profile_assignment_under5
        records = [{"ProfileId": "p01", "Name": "Standard User", "userCount": "2"}]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.permissions.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.permissions.five_or_less_profile_assignees", mock_gauge):
...
class TestProfileNoActiveUsers:
    def test_sets_gauge_per_record(self, mock_sf):
        from sfmon.tech_debt.permissions import profile_no_active_users
        records = [{"Id": "p01", "Name": "Empty Profile"}]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.permissions.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.permissions.unassigned_profiles", mock_gauge):
...
class TestMonitorMinimalPermSets:
    ...
    def test_no_report_file_exits_quietly(self, mock_sf, tmp_path):
        from sfmon.tech_debt.permissions import monitor_minimal_perm_sets
        # Point script_dir to tmp_path where there's no file
        with patch("sfmon.tech_debt.permissions.os.path.dirname", return_value=str(tmp_path)):
    ...
    def test_sets_gauge_per_perm_set(self, mock_sf, tmp_path):
        ...
        with patch("sfmon.tech_debt.permissions.os.path.dirname", return_value=str(tmp_path)), \
             patch("sfmon.tech_debt.permissions.minimal_permission_sets_gauge", mock_gauge), \
             patch("sfmon.tech_debt.permissions.minimal_permission_sets_percentage_gauge", mock_pct_gauge):

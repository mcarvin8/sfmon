...
class TestParseDatetime:
    def test_parses_valid_datetime(self):
        from sfmon.audit.deployments import parse_datetime
...
class TestCalculateMinutesDifference:
    def test_calculates_minutes(self):
        from sfmon.audit.deployments import calculate_minutes_difference
...
class TestReportDeploymentMetrics:
    ...
    def test_uses_deployment_gauges_when_not_validation(self):
        from sfmon.audit.deployments import report_deployment_metrics
        status_mapping = {"Succeeded": 1, "Failed": 0, "Canceled": -1}
        record = self._make_record(check_only=False)
        mock_detail = MagicMock()
        mock_pending = MagicMock()
        mock_time = MagicMock()
        with patch("sfmon.audit.deployments.deployment_details_gauge", mock_detail), \
             patch("sfmon.audit.deployments.pending_time_gauge", mock_pending), \
             patch("sfmon.audit.deployments.deployment_time_gauge", mock_time):
    ...
    def test_uses_validation_gauges_when_check_only(self):
        ...
        with patch("sfmon.audit.deployments.validation_details_gauge", mock_val_detail), \
             patch("sfmon.audit.deployments.validation_pending_time_gauge", mock_val_pending), \
             patch("sfmon.audit.deployments.validation_time_gauge", mock_val_time):
...
class TestProcessDeploymentRecord:
    ...
    def test_calls_report_with_calculated_times(self):
        from sfmon.audit.deployments import process_deployment_record
        record = self._make_record()
        with patch("sfmon.audit.deployments.report_deployment_metrics") as m_report:
...
class TestGetDeploymentStatus:
    ...
    def test_processes_succeeded_records(self, mock_sf):
        from sfmon.audit.deployments import get_deployment_status
        records = [self._make_record("Succeeded")]
        with patch("sfmon.audit.deployments.tooling_query_records_all", return_value=records), \
             patch("sfmon.audit.deployments.process_deployment_record") as m_process:
    ...
    def test_processes_validation_records(self, mock_sf):
        ...
        with patch("sfmon.audit.deployments.tooling_query_records_all", return_value=records), \
             patch("sfmon.audit.deployments.process_deployment_record", side_effect=capture):
            get_deployment_status(mock_sf)
        assert captured["is_validation"] is True

    def test_handles_exception(self, mock_sf):
        from sfmon.audit.deployments import get_deployment_status
        with patch("sfmon.audit.deployments.tooling_query_records_all", side_effect=RuntimeError("fail")):

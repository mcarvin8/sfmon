...
class TestGetForbiddenProfiles:
    def test_returns_empty_list_when_unset(self):
        with patch.dict(os.environ, {"FORBIDDEN_PROD_PROFILES": ""}):
            ...
            import importlib
            import sfmon.audit.forbidden_profiles as fp
            importlib.reload(fp)
...
class TestMonitorForbiddenProfileAssignments:
    def test_no_configured_profiles_sets_not_configured_label(self, mock_sf):
        from sfmon.audit.forbidden_profiles import monitor_forbidden_profile_assignments
        mock_gauge = MagicMock()
        with patch("sfmon.audit.forbidden_profiles.FORBIDDEN_PROD_PROFILES", []), \
             patch("sfmon.audit.forbidden_profiles.forbidden_profile_users_gauge", mock_gauge):
    ...
    def test_no_violations_sets_no_violations_label(self, mock_sf):
        from sfmon.audit.forbidden_profiles import monitor_forbidden_profile_assignments
        mock_gauge = MagicMock()
        with patch("sfmon.audit.forbidden_profiles.FORBIDDEN_PROD_PROFILES", ["AdminProfile"]), \
             patch("sfmon.audit.forbidden_profiles.query_records_all", return_value=[]), \
             patch("sfmon.audit.forbidden_profiles.forbidden_profile_users_gauge", mock_gauge):
            monitor_forbidden_profile_assignments(mock_sf)
        mock_gauge.labels.assert_called_once_with(
    ...
    def test_violation_sets_user_label(self, mock_sf):
        ...
        mock_gauge = MagicMock()
        with patch("sfmon.audit.forbidden_profiles.FORBIDDEN_PROD_PROFILES", ["AdminProfile"]), \
             patch("sfmon.audit.forbidden_profiles.query_records_all", return_value=[user]), \
             patch("sfmon.audit.forbidden_profiles.forbidden_profile_users_gauge", mock_gauge):
            monitor_forbidden_profile_assignments(mock_sf)
        mock_gauge.labels.assert_called_once_with(
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.audit.forbidden_profiles import monitor_forbidden_profile_assignments
        with patch("sfmon.audit.forbidden_profiles.FORBIDDEN_PROD_PROFILES", ["AdminProfile"]), \
             patch("sfmon.audit.forbidden_profiles.query_records_all", side_effect=RuntimeError("fail")), \
             patch("sfmon.audit.forbidden_profiles.forbidden_profile_users_gauge"):

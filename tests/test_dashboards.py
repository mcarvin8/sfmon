...
class TestDashboardsWithInactiveUsers:
    ...
    def test_sets_gauge_per_dashboard(self, mock_sf):
        from sfmon.tech_debt.dashboards import dashboards_with_inactive_users
        records = [self._make_record(), self._make_record(Id="dash02", Title="Ops Dashboard")]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.dashboards.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.dashboards.dashboards_with_inactive_users_gauge", mock_gauge):
    ...
    def test_null_running_user_becomes_unknown(self, mock_sf):
        ...
        with patch("sfmon.tech_debt.dashboards.query_records_all", return_value=[record]), \
             patch("sfmon.tech_debt.dashboards.dashboards_with_inactive_users_gauge", mock_gauge):
            dashboards_with_inactive_users(mock_sf)
    ...
    def test_empty_results(self, mock_sf):
        from sfmon.tech_debt.dashboards import dashboards_with_inactive_users
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.dashboards.query_records_all", return_value=[]), \
             patch("sfmon.tech_debt.dashboards.dashboards_with_inactive_users_gauge", mock_gauge):
            dashboards_with_inactive_users(mock_sf)
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.tech_debt.dashboards import dashboards_with_inactive_users
        with patch("sfmon.tech_debt.dashboards.query_records_all", side_effect=RuntimeError("fail")):

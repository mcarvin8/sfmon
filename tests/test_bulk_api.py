...
class TestSafeInt:
    def test_converts_string_int(self):
        from sfmon.ops.bulk_api import safe_int
...
class TestIntFromRow:
    def test_first_key_found(self):
        from sfmon.ops.bulk_api import int_from_row
...
class TestIsValidEntity:
    def test_valid_entity(self):
        from sfmon.ops.bulk_api import is_valid_entity
...
class TestNormalizeElfHeader:
    def test_uppercase(self):
        from sfmon.ops.bulk_api import _normalize_elf_header
...
class TestResolveElfColumn:
    def test_finds_exact_match(self):
        from sfmon.ops.bulk_api import _resolve_elf_column
...
class TestResolveElfColumnFuzzy:
    def test_finds_substring_match(self):
        from sfmon.ops.bulk_api import _resolve_elf_column_fuzzy
...
class TestReportBatchCounts:
    def test_calls_metric_per_key(self):
        from sfmon.ops.bulk_api import report_batch_counts
...
class TestReportEntityCounts:
    def test_calls_metric_per_key(self):
        from sfmon.ops.bulk_api import report_entity_counts
...
class TestProcessBulkApiLogs:
    ...
    def test_processes_bulk_api1_logs(self):
        from sfmon.ops.bulk_api import process_bulk_api_logs
...
class TestRunBulkLogAnalysis:
    def test_returns_early_when_no_rows(self, mock_sf):
        from sfmon.ops.bulk_api import _run_bulk_log_analysis
        mock_batch = MagicMock()
        mock_entity = MagicMock()
        with patch("sfmon.ops.bulk_api.query_records_all", return_value=[]), \
             patch("sfmon.ops.bulk_api.fetch_event_log_csv_reader") as m_fetch:
    ...
    def test_processes_rows_when_found(self, mock_sf):
        ...
        mock_batch = MagicMock()
        mock_entity = MagicMock()
        with patch("sfmon.ops.bulk_api.query_records_all", return_value=rows), \
             patch("sfmon.ops.bulk_api.fetch_event_log_csv_reader", return_value=reader), \
             patch("sfmon.ops.bulk_api.process_bulk_api_logs") as m_process:
            _run_bulk_log_analysis(
                mock_sf, "BulkAPI", "Hourly", mock_batch, mock_entity, "Bulk API 1.0"
            )
    ...
    def test_handles_exception(self, mock_sf):
        ...
        mock_batch = MagicMock()
        mock_entity = MagicMock()
        with patch("sfmon.ops.bulk_api.query_records_all", side_effect=RuntimeError("fail")):
            _run_bulk_log_analysis(
                mock_sf, "BulkAPI", "Hourly", mock_batch, mock_entity, "Bulk API 1.0"
            )  # Should not raise
...
class TestDailyAndHourlyAnalyseBulkApi:
    def test_daily_calls_run_twice(self, mock_sf):
        from sfmon.ops.bulk_api import daily_analyse_bulk_api
        with patch("sfmon.ops.bulk_api._run_bulk_log_analysis") as m_run:
            daily_analyse_bulk_api(mock_sf)
        assert m_run.call_count == 2

    def test_hourly_calls_run_twice(self, mock_sf):
        from sfmon.ops.bulk_api import hourly_analyse_bulk_api
        with patch("sfmon.ops.bulk_api._run_bulk_log_analysis") as m_run:
...
class TestInferNumericCountColumn:
    def test_returns_none_for_empty_rows(self):
        from sfmon.ops.bulk_api import _infer_numeric_count_column

...
class TestDormantSalesforceUsers:
    ...
    def test_sets_gauge_for_each_user(self, mock_sf):
        from sfmon.tech_debt.users import dormant_salesforce_users
        records = [self._make_user_record(), self._make_user_record(Id="005000000000002AAA", Name="Jane")]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.users.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.users.dormant_salesforce_users_gauge", mock_gauge):
    ...
    def test_null_last_login_becomes_never(self, mock_sf):
        ...
        with patch("sfmon.tech_debt.users.query_records_all", return_value=[record]), \
             patch("sfmon.tech_debt.users.dormant_salesforce_users_gauge", mock_gauge):
            dormant_salesforce_users(mock_sf)
    ...
    def test_empty_results_clears_gauge(self, mock_sf):
        from sfmon.tech_debt.users import dormant_salesforce_users
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.users.query_records_all", return_value=[]), \
             patch("sfmon.tech_debt.users.dormant_salesforce_users_gauge", mock_gauge):
            dormant_salesforce_users(mock_sf)
    ...
    def test_handles_exception_gracefully(self, mock_sf):
        from sfmon.tech_debt.users import dormant_salesforce_users
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.users.query_records_all", side_effect=RuntimeError("db error")), \
             patch("sfmon.tech_debt.users.dormant_salesforce_users_gauge", mock_gauge):
...
class TestDormantPortalUsers:
    ...
    def test_sets_gauge_for_each_portal_user(self, mock_sf):
        from sfmon.tech_debt.users import dormant_portal_users
        records = [self._make_portal_record()]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.users.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.users.dormant_portal_users_gauge", mock_gauge):
    ...
    def test_handles_exception_gracefully(self, mock_sf):
        from sfmon.tech_debt.users import dormant_portal_users
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.users.query_records_all", side_effect=Exception("fail")), \
             patch("sfmon.tech_debt.users.dormant_portal_users_gauge", mock_gauge):

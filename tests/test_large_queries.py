...
class TestIsLargeQuery:
    def test_returns_true_above_threshold(self):
        from sfmon.audit.large_queries import is_large_query
...
class TestReportLargeQueries:
    def test_sets_metric_when_queries_present(self):
        from sfmon.audit.large_queries import report_large_queries
        queries = {("u1", "Alice", "query", "Account", 50000)}
        mock_gauge = MagicMock()
        with patch("sfmon.audit.large_queries.hourly_large_query_metric", mock_gauge):
...
class TestCollectLargeQueries:
    def test_returns_empty_set_when_no_logs(self, mock_sf):
        from sfmon.audit.large_queries import collect_large_queries
        with patch("sfmon.audit.large_queries.parse_logs", return_value=None):
    ...
    def test_collects_large_queries(self, mock_sf):
        ...
        with patch("sfmon.audit.large_queries.parse_logs", return_value=iter(logs)), \
             patch("sfmon.audit.large_queries.get_user_name", return_value="Alice"):
            result = collect_large_queries(mock_sf)
        assert len(result) == 1
...
class TestHourlyObserveUserQueryingLargeRecords:
    def test_calls_collect_and_report(self, mock_sf):
        from sfmon.audit.large_queries import hourly_observe_user_querying_large_records
        with patch("sfmon.audit.large_queries.collect_large_queries", return_value=set()) as m_collect, \
             patch("sfmon.audit.large_queries.report_large_queries") as m_report:
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.audit.large_queries import hourly_observe_user_querying_large_records
        with patch("sfmon.audit.large_queries.collect_large_queries", side_effect=RuntimeError("fail")):

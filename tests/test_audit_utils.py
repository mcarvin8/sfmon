...
class TestGetUserName:
    def test_returns_name_when_found(self, mock_sf):
        from sfmon.audit.utils import get_user_name
        with patch("sfmon.audit.utils.query_records_all", return_value=[{"Name": "John Doe"}]):
            result = get_user_name(mock_sf, "001000000000001AAA")
        assert result == "John Doe"

    def test_returns_unknown_when_not_found(self, mock_sf):
        from sfmon.audit.utils import get_user_name
        with patch("sfmon.audit.utils.query_records_all", return_value=[]):
            result = get_user_name(mock_sf, "001000000000001AAA")
    ...
    def test_returns_unknown_on_exception(self, mock_sf):
        from sfmon.audit.utils import get_user_name
        with patch("sfmon.audit.utils.query_records_all", side_effect=RuntimeError("db error")):
            result = get_user_name(mock_sf, "001000000000001AAA")
        assert result == "Unknown User"

    def test_valid_15_char_id(self, mock_sf):
        from sfmon.audit.utils import get_user_name
        with patch("sfmon.audit.utils.query_records_all", return_value=[{"Name": "Jane"}]):
...
class TestCategorizeUserGroup:
    def test_integration_user_categorized(self, monkeypatch):
        ...
        import importlib
        from sfmon import audit
        importlib.reload(audit.utils)
        from sfmon.audit.utils import categorize_user_group

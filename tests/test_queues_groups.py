...
class TestTotalQueuesPerObject:
    def test_sets_gauge_per_sobject(self, mock_sf):
        from sfmon.tech_debt.queues_groups import total_queues_per_object
        records = [
            {"SobjectType": "Case", "expr0": "5"},
            {"SobjectType": "Lead", "expr0": "2"},
        ]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.queues_groups.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.queues_groups.total_queues_per_object_gauge", mock_gauge):
    ...
    def test_empty_results(self, mock_sf):
        from sfmon.tech_debt.queues_groups import total_queues_per_object
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.queues_groups.query_records_all", return_value=[]), \
             patch("sfmon.tech_debt.queues_groups.total_queues_per_object_gauge", mock_gauge):
            total_queues_per_object(mock_sf)
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.tech_debt.queues_groups import total_queues_per_object
        with patch("sfmon.tech_debt.queues_groups.query_records_all", side_effect=RuntimeError("fail")):
            total_queues_per_object(mock_sf)  # Should not raise


class TestQueuesWithNoMembers:
    def test_sets_gauge_per_queue(self, mock_sf):
        from sfmon.tech_debt.queues_groups import queues_with_no_members
        records = [{"Id": "q01", "Name": "EmptyQueue"}]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.queues_groups.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.queues_groups.queues_with_no_members_gauge", mock_gauge):
...
class TestQueuesWithZeroOpenCases:
    def test_sets_gauge_per_queue(self, mock_sf):
        from sfmon.tech_debt.queues_groups import queues_with_zero_open_cases
        records = [{"Id": "q01", "Name": "IdleQueue"}]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.queues_groups.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.queues_groups.queues_with_zero_open_cases_gauge", mock_gauge):
...
class TestPublicGroupsWithNoMembers:
    def test_sets_gauge_per_group(self, mock_sf):
        from sfmon.tech_debt.queues_groups import public_groups_with_no_members
        records = [{"Id": "g01", "Name": "EmptyGroup"}]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.queues_groups.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.queues_groups.public_groups_with_no_members_gauge", mock_gauge):

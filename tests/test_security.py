...
class TestSecurityHealthCheck:
    ...
    def test_excellent_grade(self, mock_sf):
        from sfmon.tech_debt.security import security_health_check
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.security.tooling_query_records_all", return_value=[self._make_record(95)]), \
             patch("sfmon.tech_debt.security.security_health_check_gauge", mock_gauge):
    ...
    def test_very_good_grade(self, mock_sf):
        from sfmon.tech_debt.security import security_health_check
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.security.tooling_query_records_all", return_value=[self._make_record(85)]), \
             patch("sfmon.tech_debt.security.security_health_check_gauge", mock_gauge):
            security_health_check(mock_sf)
    ...
    def test_good_grade(self, mock_sf):
        from sfmon.tech_debt.security import security_health_check
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.security.tooling_query_records_all", return_value=[self._make_record(75)]), \
             patch("sfmon.tech_debt.security.security_health_check_gauge", mock_gauge):
            security_health_check(mock_sf)
    ...
    def test_poor_grade(self, mock_sf):
        from sfmon.tech_debt.security import security_health_check
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.security.tooling_query_records_all", return_value=[self._make_record(60)]), \
             patch("sfmon.tech_debt.security.security_health_check_gauge", mock_gauge):
            security_health_check(mock_sf)
    ...
    def test_very_poor_grade(self, mock_sf):
        from sfmon.tech_debt.security import security_health_check
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.security.tooling_query_records_all", return_value=[self._make_record(40)]), \
             patch("sfmon.tech_debt.security.security_health_check_gauge", mock_gauge):
            security_health_check(mock_sf)
    ...
    def test_empty_results(self, mock_sf):
        from sfmon.tech_debt.security import security_health_check
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.security.tooling_query_records_all", return_value=[]), \
             patch("sfmon.tech_debt.security.security_health_check_gauge", mock_gauge):
            security_health_check(mock_sf)
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.tech_debt.security import security_health_check
        with patch("sfmon.tech_debt.security.tooling_query_records_all", side_effect=RuntimeError("fail")):
...
class TestSalesforceHealthRisks:
    ...
    def test_sets_mismatch_status(self, mock_sf):
        from sfmon.tech_debt.security import salesforce_health_risks
        records = [self._make_risk("enabled", "disabled")]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.security.tooling_query_records_all", return_value=records), \
             patch("sfmon.tech_debt.security.salesforce_health_risks_gauge", mock_gauge):

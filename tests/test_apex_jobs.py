...
class TestAsyncApexJobStatus:
    def test_aggregates_matching_records(self, mock_sf):
        from sfmon.ops.apex_jobs import async_apex_job_status
        records = [
        ...
        with patch("sfmon.ops.apex_jobs.query_records_all", return_value=records), \
             patch("sfmon.ops.apex_jobs.async_job_status_gauge", mock_gauge):
    ...
    def test_empty_results(self, mock_sf):
        from sfmon.ops.apex_jobs import async_apex_job_status
        mock_gauge = MagicMock()
        with patch("sfmon.ops.apex_jobs.query_records_all", return_value=[]), \
             patch("sfmon.ops.apex_jobs.async_job_status_gauge", mock_gauge):
            async_apex_job_status(mock_sf)
...
class TestMonitorApexExecutionTime:
    ...
    def test_sets_all_time_metrics(self, mock_sf):
        from sfmon.ops.apex_jobs import monitor_apex_execution_time
        logs = [self._make_log_row()]
        with patch("sfmon.ops.apex_jobs.parse_logs", return_value=iter(logs)), \
             patch("sfmon.ops.apex_jobs.run_time_metric") as m_run, \
             patch("sfmon.ops.apex_jobs.cpu_time_metric") as m_cpu, \
             patch("sfmon.ops.apex_jobs.exec_time_metric") as m_exec, \
             patch("sfmon.ops.apex_jobs.db_total_time_metric") as m_db, \
             patch("sfmon.ops.apex_jobs.callout_time_metric") as m_callout:
    ...
    def test_handles_none_logs(self, mock_sf):
        from sfmon.ops.apex_jobs import monitor_apex_execution_time
        with patch("sfmon.ops.apex_jobs.parse_logs", return_value=None), \
             patch("sfmon.ops.apex_jobs.run_time_metric") as m_run:
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.ops.apex_jobs import monitor_apex_execution_time
        with patch("sfmon.ops.apex_jobs.parse_logs", side_effect=RuntimeError("fail")):
            monitor_apex_execution_time(mock_sf)  # Should not raise
...
class TestExposeApexExceptionMetrics:
    ...
    def test_exposes_exception_detail_metric(self, mock_sf):
        from sfmon.ops.apex_jobs import expose_apex_exception_metrics
        logs = [self._make_exception_row()]
        mock_detail = MagicMock()
        mock_category = MagicMock()
        with patch("sfmon.ops.apex_jobs.parse_logs", return_value=iter(logs)), \
             patch("sfmon.ops.apex_jobs.apex_exception_details_gauge", mock_detail), \
             patch("sfmon.ops.apex_jobs.apex_exception_category_count_gauge", mock_category):
    ...
    def test_handles_empty_logs(self, mock_sf):
        ...
        mock_detail = MagicMock()
        mock_category = MagicMock()
        with patch("sfmon.ops.apex_jobs.parse_logs", return_value=iter([])), \
             patch("sfmon.ops.apex_jobs.apex_exception_details_gauge", mock_detail), \
             patch("sfmon.ops.apex_jobs.apex_exception_category_count_gauge", mock_category):
            expose_apex_exception_metrics(mock_sf)
...
class TestExposeConcurrentErrorsByAvgRuntime:
    ...
    def test_emits_gauge_per_entry_point(self):
        from sfmon.ops.apex_jobs import expose_concurrent_errors_metrics_sorted_by_average_runtime
        mock_gauge = MagicMock()
        with patch("sfmon.ops.apex_jobs.top_apex_concurrent_errors_sorted_by_avg_runtime", mock_gauge):
...
class TestExposeConcurrentErrorsByCount:
    ...
    def test_emits_gauge_per_entry_point(self):
        from sfmon.ops.apex_jobs import expose_concurrent_errors_metrics_sorted_by_request_count
        mock_gauge = MagicMock()
        with patch("sfmon.ops.apex_jobs.top_apex_concurrent_errors_sorted_by_count", mock_gauge):
...
class TestConcurrentApexErrors:
    ...
    def test_no_long_running_requests_returns_early(self, mock_sf):
        from sfmon.ops.apex_jobs import concurrent_apex_errors
    ...
    def test_long_running_requests_invoke_sub_functions(self, mock_sf):
        ...
        with patch("sfmon.ops.apex_jobs.parse_logs", return_value=iter(logs)), \
             patch("sfmon.ops.apex_jobs.expose_concurrent_errors_metrics_sorted_by_average_runtime") as m_avg, \
             patch("sfmon.ops.apex_jobs.expose_concurrent_errors_metrics_sorted_by_request_count") as m_cnt:
...
class TestExposeConcurrentLongRunningApexErrors:
    def test_no_logs_returns_early(self, mock_sf):
        from sfmon.ops.apex_jobs import expose_concurrent_long_running_apex_errors
        mock_gauge = MagicMock()
        with patch("sfmon.ops.apex_jobs.parse_logs", return_value=None), \
             patch("sfmon.ops.apex_jobs.concurrent_errors_count_gauge", mock_gauge):
...
class TestAsyncApexExecutionSummary:
    ...
    def test_sets_summary_gauges(self, mock_sf):
        from sfmon.ops.apex_jobs import async_apex_execution_summary
        logs = self._make_logs()
        mocks = {
        ...
        with patch("sfmon.ops.apex_jobs.parse_logs", return_value=iter(logs)):
            with patch.multiple("sfmon.ops.apex_jobs", **{k.split(".")[-1]: v for k, v in patches.items()}):
    ...
    def test_filters_invalid_quiddities(self, mock_sf):
        ...
        with patch("sfmon.ops.apex_jobs.parse_logs", return_value=iter(logs)), \
             patch("sfmon.ops.apex_jobs.apex_entry_point_count") as m:
            async_apex_execution_summary(mock_sf)

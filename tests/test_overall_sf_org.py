"""Unit tests for ops/overall_sf_org.py – SF and requests are mocked."""
...
class TestMonitorSalesforceLimits:
    def test_sets_gauge_for_each_limit(self, mock_sf):
        from ops.overall_sf_org import monitor_salesforce_limits
        mock_sf.limits.return_value = {
            "DailyApiRequests": {"Max": 15000, "Remaining": 14000},
            "DataStorageMB": {"Max": 5120, "Remaining": 4096},
        }
        mock_gauge = MagicMock()
        with patch("ops.overall_sf_org.api_usage_percentage_gauge", mock_gauge):
...
class TestGetSalesforceLicenses:
    def test_user_licenses_processed(self, mock_sf):
        ...
        with patch("ops.overall_sf_org.query_records_all", side_effect=[user_lic, perm_lic, usage_lic]), \
             patch("ops.overall_sf_org.total_user_licenses_gauge") as m_total, \
             patch("ops.overall_sf_org.used_user_licenses_gauge") as m_used, \
             patch("ops.overall_sf_org.percent_user_licenses_used_gauge") as m_pct, \
    ...
    def test_zero_total_licenses_skips_percent(self, mock_sf):
        ...
        with patch("ops.overall_sf_org.query_records_all", side_effect=[user_lic, [], []]), \
...
class TestFetchPod:
    def test_returns_instance_name(self, mock_sf):
        from ops.overall_sf_org import fetch_pod
        records = [{"InstanceName": "NA1"}]
        with patch("ops.overall_sf_org.query_records_all", return_value=records):

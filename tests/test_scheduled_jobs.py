...
class TestScheduledApexJobsMonitoring:
    ...
    def test_sets_gauge_per_job(self, mock_sf):
        from sfmon.tech_debt.scheduled_jobs import scheduled_apex_jobs_monitoring
        records = [self._make_record(), self._make_record(Id="cron02")]
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.scheduled_jobs.query_records_all", return_value=records), \
             patch("sfmon.tech_debt.scheduled_jobs.scheduled_apex_jobs_gauge", mock_gauge):
    ...
    def test_empty_results(self, mock_sf):
        from sfmon.tech_debt.scheduled_jobs import scheduled_apex_jobs_monitoring
        mock_gauge = MagicMock()
        with patch("sfmon.tech_debt.scheduled_jobs.query_records_all", return_value=[]), \
             patch("sfmon.tech_debt.scheduled_jobs.scheduled_apex_jobs_gauge", mock_gauge):
            scheduled_apex_jobs_monitoring(mock_sf)
    ...
    def test_handles_exception(self, mock_sf):
        from sfmon.tech_debt.scheduled_jobs import scheduled_apex_jobs_monitoring
        with patch("sfmon.tech_debt.scheduled_jobs.query_records_all", side_effect=RuntimeError("fail")):

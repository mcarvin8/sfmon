...
class TestFetchLatestLightningPageviewLog:
    def test_returns_first_record(self, mock_sf):
        from sfmon.ops.ept_apt import fetch_latest_lightning_pageview_log
        records = [{"Id": "elf001", "EventType": "LightningPageView"}]
        with patch("sfmon.ops.ept_apt.query_records_all", return_value=records):
            result = fetch_latest_lightning_pageview_log(mock_sf)
        assert result == records[0]

    def test_returns_none_when_empty(self, mock_sf):
        from sfmon.ops.ept_apt import fetch_latest_lightning_pageview_log
        with patch("sfmon.ops.ept_apt.query_records_all", return_value=[]):
            result = fetch_latest_lightning_pageview_log(mock_sf)
...
class TestDownloadLogFile:
    ...
    def test_returns_text_on_200(self):
        from sfmon.ops.ept_apt import download_log_file
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.text = "FIELD1,FIELD2\nval1,val2"
        with patch("sfmon.ops.ept_apt.requests.get", return_value=mock_resp):
...
class TestParseLogData:
    ...
    def test_accumulates_page_time(self):
        from sfmon.ops.ept_apt import parse_log_data
...
class TestUpdatePageTimeData:
    def test_accumulates_duration(self):
        from sfmon.ops.ept_apt import update_page_time_data
...
class TestReportEptMetrics:
    def test_sets_metric_per_row(self):
        from sfmon.ops.ept_apt import report_ept_metrics
        rows = [{
        ...
        mock_ept = MagicMock()
        with patch("sfmon.ops.ept_apt.ept_metric", mock_ept):
...
class TestGetSalesforceEptAndApt:
    def test_handles_no_record(self, mock_sf):
        from sfmon.ops.ept_apt import get_salesforce_ept_and_apt
        with patch("sfmon.ops.ept_apt.fetch_latest_lightning_pageview_log", return_value=None):
            get_salesforce_ept_and_apt(mock_sf)  # Should not raise

    def test_handles_no_log_data(self, mock_sf):
        from sfmon.ops.ept_apt import get_salesforce_ept_and_apt
        with patch("sfmon.ops.ept_apt.fetch_latest_lightning_pageview_log", return_value={"Id": "e01"}), \
             patch("sfmon.ops.ept_apt.download_log_file", return_value=None):
            get_salesforce_ept_and_apt(mock_sf)  # Should not raise

    def test_handles_exception(self, mock_sf):
        from sfmon.ops.ept_apt import get_salesforce_ept_and_apt
        with patch("sfmon.ops.ept_apt.fetch_latest_lightning_pageview_log", side_effect=RuntimeError("fail")):
            get_salesforce_ept_and_apt(mock_sf)  # Should not raise

    def test_full_flow(self, mock_sf):
        from sfmon.ops.ept_apt import get_salesforce_ept_and_apt
        with patch("sfmon.ops.ept_apt.fetch_latest_lightning_pageview_log", return_value={"Id": "e01"}), \
             patch("sfmon.ops.ept_apt.download_log_file", return_value="PAGE_APP_NAME,DURATION\nHome,2000"), \
             patch("sfmon.ops.ept_apt.report_apt_metrics") as m_apt, \
             patch("sfmon.ops.ept_apt.report_ept_metrics") as m_ept:

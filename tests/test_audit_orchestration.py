...
class TestRunHourlyAudit:
    def test_calls_all_hourly_functions(self, mock_sf):
        from sfmon.audit import run_hourly_audit
        with patch("sfmon.audit.audit.hourly_observe_user_querying_large_records") as m_large, \
             patch("sfmon.audit.audit.monitor_forbidden_profile_assignments") as m_fp:
...
class TestRunDailyAudit:
    def test_calls_all_daily_functions(self, mock_sf):
        from sfmon.audit import run_daily_audit
        with patch("sfmon.audit.audit.expose_suspicious_records") as m_sus, \
             patch("sfmon.audit.audit.monitor_org_wide_sharing_settings") as m_share, \
             patch("sfmon.audit.audit.get_deployment_status") as m_dep, \
             patch("sfmon.audit.audit.monitor_login_events") as m_login, \
             patch("sfmon.audit.audit.geolocation") as m_geo:

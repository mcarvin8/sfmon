...
from gauges import (
    ...
    used_permissionset_licenses_gauge,
    used_usage_based_entitlements_licenses_gauge,
    used_user_licenses_gauge,
    percent_permissionset_used_gauge,
    percent_usage_based_entitlements_used_gauge,
    percent_user_licenses_used_gauge,
    maintenance_gauge,
)
...
def get_salesforce_licenses(sf):
    ...
    for entry in result_user_license:
        ...
        total_user_licenses_gauge.labels(license_name=license_name, status=status).set(
            total_licenses
        )
        used_user_licenses_gauge.labels(license_name=license_name, status=status).set(
            used_licenses
        )

        if total_licenses != 0:
            percent_used = (used_licenses / total_licenses) * 100
            percent_user_licenses_used_gauge.labels(
    ...
    result_perm_set_license = query_records_all(
        sf,
        "SELECT MasterLabel, Status, ExpirationDate, TotalLicenses, UsedLicenses FROM PermissionSetLicense",
    )
    for entry in result_perm_set_license:
        ...
        total_permissionset_licenses_gauge.labels(
            license_name=license_name, status=status
        ).set(total_licenses)
        used_permissionset_licenses_gauge.labels(
            license_name=license_name, status=status
        ).set(used_licenses)

        if total_licenses != 0:
            percent_used = (used_licenses / total_licenses) * 100
            percent_permissionset_used_gauge.labels(
                license_name=license_name,
                status=status,
    ...
    for entry in result_usage_based_entitlements:
        ...
        total_usage_based_entitlements_licenses_gauge.labels(
            license_name=license_name
        ).set(total_licenses)
        if used_licenses:
            used_usage_based_entitlements_licenses_gauge.labels(
                license_name=license_name
            ).set(used_licenses)

        if total_licenses != 0 and used_licenses is not None:
            percent_used = (used_licenses / total_licenses) * 100
            percent_usage_based_entitlements_used_gauge.labels(
                license_name=license_name,
                expiration_date=expiration_date,
                used_licenses=used_licenses,
                total_licenses=total_licenses,
...
def get_salesforce_incidents(org, instancepod):
    ...
    try:
        response = requests.get(
            f"{SALESFORCE_STATUS_API_URL}/v1/incidents/active",
...
def get_salesforce_maintenances(pod_map):
    ...
    try:
        response = requests.get(
            f"{SALESFORCE_STATUS_API_URL}/v1/maintenances",
            timeout=REQUESTS_TIMEOUT_SECONDS,
        )
        response.raise_for_status()

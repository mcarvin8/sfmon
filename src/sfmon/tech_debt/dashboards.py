...
def dashboards_with_inactive_users(sf):
    ...
    try:
        ...
        for record in results:
            ...
            running_user_name = (
                record.get("RunningUser", {}).get("Name", "Unknown")
                if record.get("RunningUser")
                else "Unknown"
            )
            ...
            dashboards_with_inactive_users_gauge.labels(
                ...
                last_referenced_date=(
                    last_referenced_date if last_referenced_date else "Never"
                ),
            ).set(1)

        logger.info(
            "Dashboards with inactive users monitoring completed. Found %d dashboards with inactive running users",
            dashboard_count,
        )
